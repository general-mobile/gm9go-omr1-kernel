...
//#define sensor_flip
//#define GPIO_GC0310_PDN           (97+343)
#if 1
int main2_vcama_gpio = 0;
int main2_pdn_gpio = 0;
int main2_rst_gpio = 0;
#endif
...
static kal_uint32 open(void)
{
    ...
    if(node == NULL){
      printk("node gpio not find\n");
    }
    main2_vcama_gpio = of_get_named_gpio(node, "gpio_main2_vcama", 0);
    main2_pdn_gpio = of_get_named_gpio(node, "gpio_main2_pdn", 0);
    main2_rst_gpio = of_get_named_gpio(node, "gpio_main2_rst", 0);
    ret = gpio_request(main2_vcama_gpio, "mediatek,imgsensor"); 
    ret = gpio_request(main2_pdn_gpio, "mediatek,imgsensor"); 
    ret = gpio_request(main2_rst_gpio, "mediatek,imgsensor"); 
    gpio_direction_output(main2_vcama_gpio, 0);
    gpio_direction_output(main2_pdn_gpio, 0);
    gpio_direction_output(main2_rst_gpio, 0);
    gpio_set_value(main2_vcama_gpio, 1);
    gpio_set_value(main2_pdn_gpio, 0);
    gpio_set_value(main2_rst_gpio, 1);
	#ifdef CONFIG_FAKE_DUAL_CAMERA
...
#if 1
    gpio_set_value(main2_vcama_gpio, 0);
    gpio_set_value(main2_pdn_gpio, 1);
    gpio_set_value(main2_rst_gpio, 0);
#endif
// 	gpio_set_value_cansleep(GPIO_GC0310_PDN, 1);
	return ERROR_NONE;
}	/*	close  */
